...
import { CheckCircle, Lock, Play, Trophy, Diamond, ChevronLeft, ChevronRight, BookOpen, GraduationCap, Layers, Rocket, ListChecks } from "lucide-react"
...
export default function LearnGrid({
  ...
  const resolvedInitialCategory = useMemo(() => {
    const byEquality = (val?: string | null) => (val ? categories.find((c) => c === val) : undefined)
    const byCase = (val?: string | null) =>
      val ? categories.find((c) => c.toLowerCase() === val.toLowerCase()) : undefined
  ...
  const [totalCount, setTotalCount] = useState<number>(paginationMode ? (total as number) : (activities?.length ?? 0))
  ...
  const totalPages = useMemo(() => {
    ...
    return Math.max(1, Math.ceil(totalCount / (isMobile ? pageSizeMobile : pageSizeDesktop)))
  ...
  const completedCount = useMemo(() => filtered.filter((a) => a.completed).length, [filtered])
  ...
  const totalDiamondsEarned = useMemo(
    () => filtered.filter((a) => a.completed).reduce((sum, a) => sum + (a.diamondReward ?? 0), 0),
    [filtered]
  )
  ...
  return (
    <main className="max-w-md mx-auto md:max-w-4xl lg:max-w-5xl xl:max-w-6xl px-4 py-8 space-y-8">
          ...
          <CardDescription>
            ...
            {paginationMode ? (
              <span className="text-xs text-muted-foreground">
                {filtered.length} shown • {totalCount} total • {completedCount} completed
              </span>
            ) : (
              <span className="text-xs text-muted-foreground">
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {lesson.isLocked && <Lock className="w-5 h-5 text-muted-foreground" />}
                  </div>
                </div>
              </CardHeader>
              <CardContent className="pt-0">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link href={lesson.isLocked ? "#" : `/learn/${lesson.slug}`}>
                    <Button size="sm" disabled={lesson.isLocked} className="flex items-center gap-1">
                      {lesson.completed ? (
                        <>
      ...
      <Card className="overflow-hidden">
        <CardHeader className="pb-4 bg-gradient-to-r from-indigo-50 via-sky-50 to-cyan-50 dark:from-indigo-950/40 dark:via-sky-950/30 dark:to-cyan-950/20 border-b">
          <CardTitle className="font-[family-name:var(--font-work-sans)] flex items-center gap-2">
          ...
          <CardDescription>Follow this path step-by-step for the best progression</CardDescription>
        </CardHeader>
          ...
          <div className="hidden md:block">
            <div className="relative">
              ...
              <div className="grid md:grid-cols-5 gap-6">
                {[
                  { title: "Python Fundamentals", subtitle: "Variables, Types, Strings, Lists", Icon: BookOpen, gradient: "from-indigo-500 to-blue-500", badge: "Beginner" },
                  { title: "Control Flow", subtitle: "Conditions, Loops, Logic", Icon: ListChecks, gradient: "from-emerald-500 to-teal-500", badge: "Core" },
                  { title: "Functions", subtitle: "Parameters, Return, Scope", Icon: GraduationCap, gradient: "from-violet-500 to-fuchsia-500", badge: "Core" },
                  { title: "Data Structures", subtitle: "Lists, Tuples, Dicts, Sets", Icon: Layers, gradient: "from-amber-500 to-orange-500", badge: "Core" },
                  { title: "Mini Projects", subtitle: "Apply what you learned", Icon: Rocket, gradient: "from-rose-500 to-pink-500", badge: "Practice" },
                ].map((s, idx) => (
                  <div key={idx} className="relative flex flex-col items-center text-center gap-3">
                    <div className="relative z-10">
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${s.gradient} p-[2px] shadow-sm`}>
                    ...
                    <div className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary">{s.badge}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="md:hidden space-y-5">
            {[
              ...
              <div key={idx} className="flex items-start gap-3">
                <div className="relative">
                  <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${s.gradient} p-[2px]`}>
                    <div className="w-full h-full rounded-full bg-background flex items-center justify-center">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <div className="text-sm font-medium">{s.title}</div>
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary">{s.badge}</span>
                  </div>
                  <div className="text-xs text-muted-foreground">{s.subtitle}</div>
                </div>
              </div>
