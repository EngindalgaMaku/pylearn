...
export default function FunctionCallsGame() {
  ...
  useEffect(() => {
    const post = async () => {
      ...
      try {
        await fetch("/api/games/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ gameKey: "function-calls", score, correctCount: score, durationSec, bonusXP }) })
      } catch (e) {
