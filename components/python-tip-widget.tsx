...
export default function PythonTipWidget({
  ...
  const prevExperience = useMemo(
    () => (session?.user as any)?.experience ?? 0,
    [session]
  );
  const prevDiamonds = useMemo(
    () => (session?.user as any)?.currentDiamonds ?? 0,
    [session]
  );
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const statusUrl =
          mode === "tips"
            ? "/api/python-tips/complete-tip"
            : "/api/python-tips/complete";
        const res = await fetch(statusUrl, { cache: "no-store" });
        if (res.status === 401) {
          // Don't auto-open login dialog on either homepage or tips page.
        ...
        if (!res.ok) {
          const msg = await res
            .text()
            .catch(() => "Failed to check daily tip status");
        ...
        toast({
          title: "Network error",
          description: "Could not check daily tip status.",
        });
      } finally {
  ...
  return (
    <Card className="w-full bg-gradient-to-br from-slate-900 to-slate-800 text-white border-slate-700">
      <CardHeader className="p-4 md:p-6 pb-2 md:pb-3">
        ...
        <Badge
          variant="outline"
          className="w-fit text-[10px] md:text-xs px-1.5 py-0.5 border-slate-600 text-slate-300"
        >
          {tip.category}
        </Badge>
      </CardHeader>
        ...
        <DialogContent className="bg-gradient-to-br from-rose-900 to-rose-800 text-white border-rose-700">
          <DialogHeader>
            ...
            <DialogDescription className="text-rose-100">
              You can earn rewards from up to 3 tips per day. Come back tomorrow
              for more XP!
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
