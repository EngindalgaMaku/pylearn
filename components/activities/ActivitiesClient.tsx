...
import { Play, Lock, CheckCircle, Diamond, Zap, Clock, Trophy, Target, BookOpen, Code2, FlaskConical, Puzzle, Database, LineChart, MonitorPlay, HelpCircle } from "lucide-react"
...
const topicColorPool = ["bg-blue-500", "bg-green-500", "bg-purple-500", "bg-orange-500", "bg-pink-500", "bg-teal-500"]
...
export default function ActivitiesClient({
  ...
  const categoriesMap = useMemo(() => {
    const map: Record<
      string,
      { id: string; title: string; description: string; color: string; totalActivities: number; completedActivities: number; progress: number }
    > = {}
    const catList = Array.from(new Set(normalized.map((a) => a.category).filter(Boolean)))
    catList.forEach((cat, idx) => {
      ...
      map[cat] = {
        id: cat,
        title: cat,
        description: CATEGORY_EXPLANATIONS[cat] ?? `Explore ${cat} through interactive activities`,
  ...
  const overallProgress = total > 0 ? Math.round((completedActivities / total) * 100) : 0
  ...
  const topicList = useMemo(() => {
    ...
    const list = canonTitles.map((title, idx) => {
      ...
      return {
        id: title,
        title,
        description: CATEGORY_EXPLANATIONS[title] ?? `Explore ${title} through interactive activities`,
        color: topicColorPool[idx % topicColorPool.length],
  ...
  const selectedProgress = useMemo(() => {
    ...
    return selectedCategoryInfo ? Math.round((selectedCategoryInfo.completedActivities / denom) * 100) : 0
  }, [selectedCategoryInfo, total])

  // Build URLs for server-side navigation
  const buildHref = (category: string | null, type: string | null, pageNum: number) => {
  ...
  const typeList = useMemo(() => {
    const base = Array.from(
      new Set((activityTypes || []).map((t) => (t || "").trim()).filter(Boolean))
    )
      ...
      <div className="max-w-4xl mx-auto lg:max-w-6xl xl:max-w-7xl px-4 py-6 md:px-6 md:py-8 lg:px-8">
          ...
          <div className="mb-4 max-w-sm">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-serif font-semibold text-lg text-foreground">Activity Type</h3>
              </div>
              <Select
              ...
              <Card className="hover:shadow-xl transition-all duration-300 border-border/50">
                <CardHeader className="pb-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className={`w-4 h-4 rounded-full ${selectedCategoryInfo.color} shadow-lg`} />
                  ...
                  <CardDescription className="text-muted-foreground">{selectedCategoryInfo.description}</CardDescription>
                </CardHeader>
                <CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredActivities.map((activity) => {
            const typeColor =
              activityTypeColors[activity.activityType] ?? activityTypeColors[activity.activityType?.toLowerCase()] ?? "bg-slate-100 text-slate-800"
            return (
              <Card
                key={activity.id}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-3 flex-wrap">
                        <Badge className={`${typeColor} font-medium flex items-center gap-1`}>
                      ...
                      {activity.description && (
                        <CardDescription className="mt-2 text-muted-foreground leading-relaxed">{activity.description}</CardDescription>
                      )}
                    </div>
                  </div>
                </CardHeader>
          ...
          <div className="flex gap-2">
            {canPrev ? (
              <Button asChild variant="outline">
                <Link href={buildHref(selectedTopic, selectedActivityType, page - 1)}>Previous</Link>
              </Button>
            ) : (
            ...
            {canNext ? (
              <Button asChild variant="outline">
                <Link href={buildHref(selectedTopic, selectedActivityType, page + 1)}>Next</Link>
              </Button>
            ) : (
              <Button variant="outline" disabled>
                Next
