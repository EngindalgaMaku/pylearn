...
export default function FillBlanksRunner({
  ...
  async function handleClaimRewards(scorePct?: number, timeSpentOverride?: number) {
    ...
    try {
      ...
      // Update session with server snapshot
      ...
      if (userAfter && typeof update === "function") {
        try {
          ...
          console.warn("Session update after fill-blanks completion failed:", e)
        }
      }
      ...
      // Do not auto-redirect; user will navigate via dialog buttons
    } catch (error) {
