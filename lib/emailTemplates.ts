...
export function resetPasswordEmailHTML(params: {
  ...
  // Use PNG for broad email client support (Gmail blocks external SVGs)
  const logoUrl = params.logoUrl || `${base}/placeholder-logo.png`
...
export function welcomeEmailHTML(params: {
  ...
  const logoUrl = params.logoUrl || `${base}/placeholder-logo.png`
  const supportEmail = params.supportEmail || "info@pylearn.net"
  const dashboardUrl = params.dashboardUrl || base
  const features = params.features && params.features.length
    ? params.features
    : [
        "Interactive, game-like challenges to keep you engaged",
        "Track XP and diamonds as you learn and level up",
        "Bite-sized lessons and guided activities",
        "Practice with real-world examples and instant feedback",
      ]

  const greeting = params.userName ? `Welcome, ${params.userName}!` : `Welcome to ${siteName}!`
  ...
  <html>
    <head>
      ...
      <style>
        ...
        a.btn { background: #16a34a; color: #fff !important; text-decoration: none; padding: 12px 18px; border-radius: 8px; display: inline-block; font-weight: 600; }
        .muted { color: #6b7280; }
        .card { border: 1px solid #e5e7eb; border-radius: 12px; }
              ...
              <tr>
                <td style=\"padding:0 16px;\">
                  <table width=\"100%\" role=\"presentation\" cellpadding=\"0\" cellspacing=\"0\" class=\"card\" style=\"background:#ffffff\">
                    ...
                    <tr>
                      <td class=\"content\" style=\"padding:0 32px 24px 32px;\">
                        <h1 style=\"margin:0 0 8px 0;font-size:24px;line-height:1.3;\">${greeting}</h1>
                        <p style=\"margin:0 0 12px 0;color:#334155;font-size:15px;\">
                          You're all set! Here’s what you can expect as a new learner on ${siteName}:
                        </p>
                        <ul style=\"margin:0 0 16px 0;color:#334155;font-size:15px;\">
                          ${features.map((f: string) => `<li>${f}</li>`).join("")}
                        </ul>
                        <p style=\"margin:0 0 20px 0;\">
                          <a class=\"btn\" href=\"${dashboardUrl}\" target=\"_blank\" rel=\"noopener noreferrer\">Start learning now</a>
                        </p>
                        <p class=\"muted\" style=\"margin:0 0 12px 0;font-size:13px;\">Tip: Bookmark your dashboard to come back anytime.</p>
                      </td>
                    </tr>
                    <tr>
                      <td class=\"content\" style=\"padding:16px 32px 28px 32px;border-top:1px solid #e5e7eb;\">
                        <p class=\"muted\" style=\"margin:0;font-size:12px;\">Questions? Reach us at <a href=\"mailto:${supportEmail}\" style=\"color:#2563eb;text-decoration:underline;\">${supportEmail}</a>.</p>
                      </td>
                    </tr>
                  </table>
                  <div style=\"text-align:center;margin-top:16px;color:#94a3b8;font-size:12px;\">
