...
> = {
  ...
  Legendary: {
    badge:
      "bg-gradient-to-r from-amber-400 via-yellow-500 to-orange-500 text-black font-semibold shadow-md",
  ...
  Mythic: {
    badge:
      "bg-gradient-to-r from-rose-600 via-red-500 to-orange-500 text-white font-semibold shadow-md",
...
function getElementStyle(el?: string | null) {
  ...
  if (["flame", "ember", "inferno"].includes(v))
    return { icon: "🔥", bg: "bg-red-500/90", ring: "ring-1 ring-red-400/70" };
  if (["ocean", "sea", "aqua"].includes(v))
    return {
      icon: "💧",
      bg: "bg-blue-500/90",
      ring: "ring-1 ring-blue-400/70",
    };
  if (["rock", "stone", "ground", "terra"].includes(v))
    return {
      icon: "🪨",
      bg: "bg-amber-700/90",
      ring: "ring-1 ring-amber-600/70",
    };
  if (["wind", "breeze", "sky"].includes(v))
    return { icon: "🌬️", bg: "bg-sky-400/90", ring: "ring-1 ring-sky-300/70" };
  if (["frost", "snow", "glacier"].includes(v))
    return {
      icon: "❄️",
      bg: "bg-cyan-500/90",
      ring: "ring-1 ring-cyan-300/70",
    };
  if (["holy", "radiant"].includes(v))
    return {
      icon: "☀️",
      bg: "bg-yellow-400/90",
      ring: "ring-1 ring-yellow-300/70",
    };
  if (["dark", "void"].includes(v))
    return {
      icon: "🌑",
      bg: "bg-gray-800/90",
      ring: "ring-1 ring-gray-600/70",
    };
  if (["electric", "thunder", "storm", "lightning"].includes(v))
    return {
      icon: "⚡",
      bg: "bg-yellow-500/90",
...
export default function ShopPage() {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<CategorySlug>("anime-collection");
          ...
          categories.map(async (cat) => {
            try {
              const res = await fetch(
                `/api/cards?category=${cat.value}&page=1&pageSize=1`,
                { cache: "no-store" }
              );
              ...
              return {
                slug: cat.value,
                img: first?.image || "/placeholder.svg",
              };
            } catch {
  ...
  const handlePurchase = async (card: ShopCard) => {
    ...
    if (ownedCards.includes(card.id)) {
      toast({
        title: "Already owned",
        description: `You already own ${card.name}.`,
      });
      return;
    }
    if (!canAfford) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-slate-50/30 to-blue-50/30">
      {/* Header */}
      <header className="bg-gradient-to-r from-card via-slate-50/80 to-blue-50/80 border-b border-border/60 px-4 py-6 md:hidden backdrop-blur-sm">
        <div className="max-w-md mx-auto flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Spend your diamonds on awesome cards!
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-md mx-auto md:max-w-4xl lg:max-w-6xl px-4 py-8 space-y-6">
        ...
        <div className="hidden md:block text-center mb-8">
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto">
            Discover and collect amazing cards with your diamonds
          </p>
        </div>
        {/* Diamond Balance */}
            ...
            <button
              ...
              className={`relative aspect-[16/9] rounded-lg overflow-hidden border-2 transition-all duration-300 ${
                selectedCategory === c.value
                  ? "ring-2 ring-blue-500 shadow-lg shadow-blue-200/50 border-blue-300"
                  : "hover:shadow-lg hover:shadow-slate-200/50 border-slate-200 hover:border-slate-300"
              }`}
            >
              <img
              ...
              return (
                <Card
                  ...
                  className={`relative overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
                    ...
                    isOwned
                      ? "ring-2 ring-green-500 bg-gradient-to-br from-green-50/80 via-emerald-50/60 to-teal-50/80 shadow-md shadow-green-200/30"
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      const preview =
                        (card as any).securePreviewUrl ||
                        (card as any).secureFullImageUrl ||
                        card.image;
                      if (!preview || preview.includes("placeholder.svg"))
                        return;
                  ...
                  <CardContent className="pt-0">
                    ...
                    {card.specialAbility &&
                      card.specialAbility.trim().length > 0 && (
                    ...
                    {(card.attackPower != null ||
                      ...
                      <div className="mb-3 flex flex-wrap items-center gap-2">
                        ...
                        {es && (
                          <div
                            className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs text-white ${es.bg} ${es.ring}`}
                          >
                            <span>{es.icon}</span>
                          </div>
                        )}
                    ...
                    <div className="flex items-center justify-end">
                      <Button
                        ...
                        disabled={
                          isOwned || !canAfford || purchaseLoadingId === card.id
                        }
                        size="sm"
                        className={`${
        ...
        {/* Collection Stats */}
        <Card className="bg-gradient-to-br from-purple-50/80 via-blue-50/60 to-indigo-50/80 border-blue-200/60 shadow-lg hover:shadow-xl transition-all duration-300">
          <CardHeader>
