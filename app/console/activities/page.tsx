...
function getTypeColor(type: string | null | undefined): string {
  ...
  if (typeStr.includes("quiz")) return "bg-emerald-100 text-emerald-800";
  if (typeStr.includes("interactive") || typeStr.includes("interactive_demo"))
    return "bg-blue-100 text-blue-800";
  if (typeStr.includes("theory_interactive"))
    return "bg-purple-100 text-purple-800";
  if (typeStr.includes("matching")) return "bg-purple-100 text-purple-800";
  if (typeStr.includes("memory") || typeStr.includes("memory_game"))
    return "bg-pink-100 text-pink-800";
  if (typeStr.includes("drag") || typeStr.includes("drag_drop"))
    return "bg-orange-100 text-orange-800";
  if (typeStr.includes("code") || typeStr.includes("coding"))
    return "bg-green-100 text-green-800";
  if (typeStr.includes("algorithm")) return "bg-red-100 text-red-800";
  if (typeStr.includes("data") || typeStr.includes("exploration"))
...
export default async function ActivitiesPage({
  ...
  const q = (
    (sp?.q ?? (typeof sp?.get === "function" ? sp.get("q") : "")) ||
    ""
  ).trim();
  const category = (
    (sp?.category ??
      (typeof sp?.get === "function" ? sp.get("category") : "")) ||
    ""
  ).trim();
  const typeGroup = String(
    sp?.typeGroup ?? (typeof sp?.get === "function" ? sp.get("typeGroup") : "")
  )
    .trim()
    .toLowerCase();
  const activityType = (
    (sp?.activityType ??
      (typeof sp?.get === "function" ? sp.get("activityType") : "")) ||
    ""
  ).trim();
  const sizeRaw = Number(
    sp?.size ?? (typeof sp?.get === "function" ? sp.get("size") : 20)
  );
  const pageRaw = Number(
    sp?.page ?? (typeof sp?.get === "function" ? sp.get("page") : 1)
  );
  const sort = String(
    sp?.sort ?? (typeof sp?.get === "function" ? sp.get("sort") : "updatedAt")
  ).trim();
  const dirRaw = String(
    sp?.dir ?? (typeof sp?.get === "function" ? sp.get("dir") : "desc")
  )
    .trim()
    .toLowerCase();
  const size = Math.min(
    100,
    Math.max(5, Number.isFinite(sizeRaw) && sizeRaw ? sizeRaw : 20)
  );
    ...
    case "games":
      typeWhere = {
        OR: [
          ...
          {
            activityType: {
              in: ["matching", "memory_game", "drag_drop"],
              mode: Prisma.QueryMode.default as any,
            },
          },
        ],
      } as any;
  ...
  const activityTypeWhere = activityType
    ? { activityType: { equals: activityType } }
    : {};

  const where: Prisma.LearningActivityWhereInput | undefined =
    q || category || typeGroup || activityType
      ? {
          AND: [
            q
              ? {
    ...
    <div className="px-4 py-6 max-w-5xl mx-auto">
              ...
              <th className="text-left px-3 py-3 font-semibold">
                <a
                  href={`?q=${encodeURIComponent(
                    ...
                    category
                  )}&typeGroup=${encodeURIComponent(
                    typeGroup
                  )}&size=${size}&page=1&sort=title&dir=${
                    sortKey === "title" && dir === "asc" ? "desc" : "asc"
                  }`}
                  className="inline-flex items-center gap-1 hover:underline text-white"
                >
                  Başlık{" "}
                  {sortKey === "title" ? (dir === "asc" ? "▲" : "▼") : ""}
                </a>
              </th>
              ...
              <th className="text-left px-3 py-3 font-semibold">
                <a
                  href={`?q=${encodeURIComponent(
                    ...
                    typeGroup
                  )}&size=${size}&page=1&sort=difficulty&dir=${
                    sortKey === "difficulty" && dir === "asc" ? "desc" : "asc"
                  }`}
                  className="inline-flex items-center gap-1 hover:underline text-white"
                >
                  Zorluk{" "}
              ...
              <th className="text-left px-3 py-3 font-semibold">
                <a
                  href={`?q=${encodeURIComponent(
                    ...
                    typeGroup
                  )}&size=${size}&page=1&sort=experienceReward&dir=${
                    sortKey === "experienceReward" && dir === "asc"
                      ? "desc"
                      : "asc"
                  }`}
                  className="inline-flex items-center gap-1 hover:underline text-white"
              ...
              <th className="text-left px-3 py-3 font-semibold">
                <a
                  href={`?q=${encodeURIComponent(
                    ...
                    typeGroup
                  )}&size=${size}&page=1&sort=diamondReward&dir=${
                    sortKey === "diamondReward" && dir === "asc"
                      ? "desc"
                      : "asc"
                  }`}
                  className="inline-flex items-center gap-1 hover:underline text-white"
          ...
          <tbody>
            {activities.map((a, idx) => (
              <tr
                ...
                } hover:bg-gradient-to-r hover:from-blue-100/60 hover:to-indigo-100/60 transition-colors`}
              >
                <td className="px-3 py-3 align-top text-muted-foreground">
                  {(page - 1) * size + idx + 1}
                </td>
                <td className="px-3 py-3 align-top">
                  <div className="font-medium truncate max-w-[320px] mb-1">
                    {a.title}
                  </div>
                  <div className="mt-1">
                ...
                <td className="px-3 py-3 align-top">
                  {typeof a.experienceReward === "number"
                    ? a.experienceReward
                    : "-"}
                </td>
                <td className="px-3 py-3 align-top">
                  {typeof a.diamondReward === "number" ? a.diamondReward : "-"}
                </td>
                <td className="px-3 py-3 align-top">
                  <span
                ...
                <td className="px-3 py-3 align-top">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    >
                      <button
                        className="h-8 px-2 rounded border text-xs hover:bg-gray-100"
                        type="submit"
                      >
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        <div className="text-muted-foreground">
          Sayfa {page} / {totalPages} • Toplam {total} aktivite
        </div>
        <div className="flex items-center gap-2">
          <a
            ...
            href={`?q=${encodeURIComponent(q)}&category=${encodeURIComponent(
              ...
              activityType
            )}&size=${size}&sort=${encodeURIComponent(
              sortKey
            )}&dir=${encodeURIComponent(dir)}&page=${Math.max(1, page - 1)}`}
          >
            Önceki
          </a>
          <a
            ...
            href={`?q=${encodeURIComponent(q)}&category=${encodeURIComponent(
              ...
              sortKey
            )}&dir=${encodeURIComponent(dir)}&page=${Math.min(
              totalPages,
              page + 1
            )}`}
          >
...
async function BulkDifficultyForm({
  q,
  category,
}: {
  q: string;
  ...
  async function bulkUpdate(formData: FormData) {
    ...
    await prisma.learningActivity.updateMany({
      where,
      data: { difficulty: newDiff },
    });
  }
  return (
    <form
      action={bulkUpdate}
      className="p-3 border rounded-md bg-muted/20 flex items-center gap-2 flex-1"
    >
