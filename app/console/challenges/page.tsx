...
export default async function ChallengesAdminPage({ searchParams }: { searchParams?: any }) {
  ...
  const [total, items, typeRows] = await Promise.all([
    ...
    prisma.weeklyChallenge.groupBy({ by: ["challengeType"], _count: { challengeType: true }, orderBy: { challengeType: "asc" } }),
  ]);
  ...
  const makeHref = (params: Record<string, string | number>) => {
    const usp = new URLSearchParams({ q, sort, dir, size: String(size), page: String(page), type: typeFilter });
  ...
  const badge = (text: string, tone: "rose" | "emerald" | "indigo" | "slate" | "amber" = "slate") => {
    ...
    return <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium ${tones[tone]} whitespace-nowrap`}>{text}</span>;
  };

  const toneForType = (t?: string | null): "rose" | "emerald" | "indigo" | "slate" | "amber" => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <form method="get" className="flex items-center gap-2">
            ...
            />
            <select name="type" defaultValue={typeFilter} className="h-9 rounded border px-2">
      ...
      <div className="rounded-xl border overflow-hidden shadow-sm">
        <div className="bg-gradient-to-r from-amber-600/90 via-orange-600/90 to-rose-600/90 text-white px-4 py-3">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((c, idx) => (
              <tr key={c.id} className={idx % 2 ? "bg-muted/20" : "bg-white hover:bg-muted/10 transition-colors"}>
                <td className="px-3 py-2">{(skip + idx + 1).toString()}</td>
                <td className="px-3 py-2">
                  <div className="font-medium">{c.title}</div>
                  <div className="text-xs text-muted-foreground">{c.description}</div>
                </td>
                <td className="px-3 py-2">{badge(c.challengeType, toneForType(c.challengeType))}</td>
                <td className="px-3 py-2 text-xs text-muted-foreground">
                  <span title={(() => { try { return (c as any).requirements } catch { return '' } })()}>
                    {(() => {
                      try {
                        const req = typeof (c as any).requirements === 'string' ? JSON.parse((c as any).requirements) : (c as any).requirements
                ...
                <td className="px-3 py-2">{c.isActive ? badge("Active", "emerald") : badge("Inactive", "slate")}</td>
                <td className="px-3 py-2 text-right space-x-2">
                  {/* Toggle with confirmation */}
                  <form id={`toggleForm-${c.id}`} method="post" action={`/api/admin/challenges/toggle?id=${c.id}`} className="hidden" />
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      ...
                      >
                        <Power className={`h-4 w-4 ${c.isActive ? "text-amber-600" : "text-emerald-600"}`} />
                      </button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>{c.isActive ? "Deactivate challenge?" : "Activate challenge?"}</AlertDialogTitle>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction form={`toggleForm-${c.id}`} type="submit">Confirm</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>

                  {/* Delete with confirmation */}
                  <form id={`deleteForm-${c.id}`} method="post" action={`/api/admin/challenges/delete?id=${c.id}`} className="hidden" />
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <button
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction form={`deleteForm-${c.id}`} type="submit">Delete</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </td>
